...
// Bestehende Daten und Module bleiben unverändert erhalten.
// =========================================================
(() => {
  const GOLD = '#b8924f';
  const CREAM = '#f6f1e7';
  ...
  function getLifePhaseSummary() {
    ...
    const headings = [...node.querySelectorAll('h3, strong')]
      .map(el => el.textContent.trim())
      .filter(Boolean);
    return {
      title: headings[0] || 'Aktuelle Lebensphase',
      text: headings[1] || 'Deine aktuelle Orientierung ansehen.'
    };
  }
  ...
  function getTriggerSummary() {
    ...
    return {
      title: heading?.textContent?.trim() || 'Trigger & Muster',
      text: paragraphs[0] || 'Deine Erkenntnisse ansehen.'
    };
  }
  ...
  function getActionSummary() {
    ...
    const paragraphs = [...node.querySelectorAll('p')].map(p => p.textContent.trim()).filter(Boolean);
    return {
      title: heading?.textContent?.trim() || 'Handlung & Integration',
      text: paragraphs[0] || 'Deine Umsetzung und Integration ansehen.'
    };
  }

  function card(icon, label, title, text, detail) {
    return `
      <button type="button" data-ics-detail="${detail}" style="width:100%;text-align:left;cursor:pointer;border:1px solid rgba(184,146,79,.34);border-radius:16px;background:rgba(184,146,79,.035);padding:16px;color:${CREAM};font:inherit;">
  ...
  function createShell() {
    ...
    shell.innerHTML = `
      ...
      <div id="icsCockpitDetail" hidden></div>
    `;
  ...
  function storeExistingDetails() {
    ...
    [
      'icsLifeCycleOverview',
      'icsLatestEnergy',
      'icsLatestCloudActivity',
      'icsLatestTrigger',
      'icsLatestMentorInsight',
      'icsLatestActionIntegration'
    ].forEach(id => {
      const node = document.getElementById(id);
      if (node && node.parentElement !== vault) vault.appendChild(node);
    });
  }
  ...
  function detailTitle(type) {
    return ({
      life: 'Deine Lebensphase',
      energy: 'Dein aktueller Zustand',
      trigger: 'Trigger & Muster',
      journal: 'Dein Journal-Impuls',
      mentor: 'Deine Mentor-Erkenntnis',
      action: 'Deine ACTION',
      integration: 'Handlung & Integration'
    })[type] || 'Dein ICS';
  }
  ...
  function openDetail(type) {
    ...
    const node = detailNode(type);
    if (!shell || !home || !detail || !node) return;

    detail.innerHTML = `
      <button type="button" id="icsCockpitBack" style="border:0;background:none;color:${GOLD};padding:0;cursor:pointer;font:inherit;font-weight:700;">← Zurück zu Mein ICS</button>
    ...
    detail.querySelector('#icsCockpitDetailContent').appendChild(node);
    home.hidden = true;
    detail.hidden = false;
  ...
  function backHome() {
    ...
    if (content) [...content.children].forEach(node => vault.appendChild(node));
    renderHome();
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (detailButton) {
      openDetail(detailButton.dataset.icsDetail);
      return;
    }
    if (event.target.closest('#icsCockpitBack')) {
      backHome();
      return;
    }
    if (event.target.closest('[data-view="meinics"]')) {
      window.setTimeout(renderHome, 180);
    }
  });

  const timer = window.setInterval(() => {
    if (renderHome()) window.clearInterval(timer);
  }, 300);
